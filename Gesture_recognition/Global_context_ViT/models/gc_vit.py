...
class GlobalQueryGen(nn.Module):
    ...
    def forward(self, x):
        """
        Forward pass with robust tensor reshaping.
        
        This method has been updated to handle tensor size mismatches that were causing
        the "shape '[1, 1, 256, 6, 32]' is invalid for input of size 9408" error.
        
        The fix includes:
        1. Dynamic calculation of actual spatial dimensions (N) instead of relying on window_size^2
        2. Automatic adjustment of dim_head when channel dimensions don't match
        3. Multiple fallback reshaping strategies for edge cases
        4. Validation of dimensions before reshaping operations
        
        Args:
            x: Input tensor of shape (B, H, W, C)
            
        Returns:
            Reshaped tensor of shape (B, 1, num_heads, N, dim_head)
        """
        x = _to_channel_last(self.to_q_global(x))
        B = x.shape[0]
        
        # Calculate the actual number of spatial positions
        H, W, C = x.shape[1], x.shape[2], x.shape[3]
        actual_N = H * W
        
        # Use the actual spatial dimensions instead of window_size^2
        # This ensures compatibility with different input resolutions
        if actual_N != self.N:
            N = actual_N
        else:
            N = self.N
        ...
        expected_C = self.num_heads * self.dim_head
        if C != expected_C:
        ...
        if N <= 0 or self.num_heads <= 0 or dim_head <= 0:
            # Fallback to safe dimensions
            N = max(1, N)
            num_heads = max(1, self.num_heads)
            dim_head = max(1, C // num_heads)
        
        # Reshape with proper error handling
        try:
            x = x.reshape(B, 1, N, self.num_heads, dim_head).permute(0, 1, 3, 2, 4)
        except RuntimeError:
            # Fallback: reshape to compatible dimensions
            total_elements = B * N * C
            
            # Try to find compatible dimensions
            if total_elements % (B * self.num_heads) == 0:
                dim_head = total_elements // (B * self.num_heads)
                try:
                    x = x.reshape(B, 1, N, self.num_heads, dim_head).permute(0, 1, 3, 2, 4)
                except:
                    # If still fails, use a more conservative approach
                    x = x.reshape(B, 1, N, C)
                    x = x.unsqueeze(3)  # Add num_heads dimension
