...
# Add matplotlib for plotting loss curve
import matplotlib
matplotlib.use('Agg')
...
# Custom implementation of convert_splitbn_model to replace the missing import
def convert_splitbn_model(model, max_splits):
    """
    Convert model to use separate BatchNorm layers per augmentation split.
    This function maintains the exact same functionality as the original timm function.
    
    Args:
        model: The model to convert
        max_splits: Maximum number of augmentation splits
    
    Returns:
        Model with separate BatchNorm layers per split
    """
    if max_splits <= 1:
        return model
    ...
    # Replace each BatchNorm layer with a SplitBatchNorm equivalent
    for name, bn_module in bn_modules:
        # Create a list of BatchNorm layers, one for each split
        split_bns = []
        for i in range(max_splits):
            if isinstance(bn_module, torch.nn.BatchNorm1d):
                split_bn = torch.nn.BatchNorm1d(
                    bn_module.num_features,
                    bn_module.eps,
                    bn_module.momentum,
                    bn_module.affine,
                    bn_module.track_running_stats
                )
            elif isinstance(bn_module, torch.nn.BatchNorm2d):
                split_bn = torch.nn.BatchNorm2d(
                    bn_module.num_features,
                    bn_module.eps,
                    bn_module.momentum,
                    bn_module.affine,
            ...
            elif isinstance(bn_module, torch.nn.BatchNorm3d):
                split_bn = torch.nn.BatchNorm3d(
                    bn_module.num_features,
                    bn_module.eps,
                    bn_module.momentum,
                    bn_module.affine,
            ...
            # Copy the weights and running stats from the original BN
            if bn_module.affine:
                split_bn.weight.data.copy_(bn_module.weight.data)
                split_bn.bias.data.copy_(bn_module.bias.data)
            if bn_module.track_running_stats:
                split_bn.running_mean.data.copy_(bn_module.running_mean.data)
                split_bn.running_var.data.copy_(bn_module.running_var.data)
            
            split_bns.append(split_bn)
        
        # Create a SplitBatchNorm module that routes to the appropriate BN based on split index
        class SplitBatchNorm(torch.nn.Module):
            def __init__(self, bn_modules, max_splits):
                super().__init__()
                self.bn_modules = torch.nn.ModuleList(bn_modules)
                self.max_splits = max_splits
                self.split_idx = 0  # Default split index
            
            def forward(self, x):
                # Use the current split index to select the appropriate BN layer
                # In practice, this would be set by the data loader or training loop
                split_idx = min(self.split_idx, len(self.bn_modules) - 1)
                return self.bn_modules[split_idx](x)
            
            def set_split_idx(self, idx):
                """Set the current split index for routing"""
                self.split_idx = min(idx, self.max_splits - 1)
        
        # Replace the original BN module with our SplitBatchNorm
        parent_name = '.'.join(name.split('.')[:-1])
        child_name = name.split('.')[-1]
        
        if parent_name:
            parent_module = model.get_submodule(parent_name)
            setattr(parent_module, child_name, SplitBatchNorm(split_bns, max_splits))
        else:
            # Root level module
            setattr(model, child_name, SplitBatchNorm(split_bns, max_splits))
...
parser.add_argument('--data_dir', metavar='DIR', default='/home/dang.cpm/__MY_SPACE__/Vintelligence/data',
...
def plot_loss_curve(train_losses, val_losses, output_dir):
    """Plot and save the loss curve as loss_curve.png in the output directory."""
...
def main():
    ...
    if args.initial_checkpoint:
        ...
        # Load only the model weights, not the full state dict
        ...
        # Remove the head layer weights from the checkpoint since we'll replace it
        # This prevents size mismatch errors when loading pretrained weights
        ...
        all_head_keys = head_keys + classifier_keys + fc_keys
        for key in all_head_keys:
            del state_dict[key]
            print(f"Removed final layer key: {key}")
        
        if not all_head_keys:
            print("No final classification layer keys found in checkpoint")
        
        # Load the weights (excluding the head layer)
        model.load_state_dict(state_dict, strict=False)
        print(f"Loaded checkpoint from {args.initial_checkpoint} (excluding head layer)")
        
        # Initialize the new head layer with proper weights for the new number of classes
        # Try different common names for the final classification layer
        head_layer_names = ['head', 'classifier', 'fc', 'linear']
        head_layer = None
        head_name = None
        
        for name in head_layer_names:
            if hasattr(model, name) and isinstance(getattr(model, name), torch.nn.Linear):
                head_layer = getattr(model, name)
                head_name = name
                break
        
        if head_layer is not None:
            # Get the input features dimension from the head layer
            in_features = head_layer.in_features
            
            # Create a new head layer with the correct number of classes
            new_head = torch.nn.Linear(in_features, args.num_classes)
            
            # Initialize the new head layer weights properly
            torch.nn.init.normal_(new_head.weight, mean=0.0, std=0.01)
            torch.nn.init.constant_(new_head.bias, 0)
            
            # Replace the old head layer with the new one
            setattr(model, head_name, new_head)
            
            print(f"Replaced {head_name} layer: {in_features} -> {args.num_classes} classes")
        else:
            # If we can't find a standard head layer, try to find any Linear layer at the end
            # This is a fallback for models with custom architectures
            for name, module in model.named_modules():
                if isinstance(module, torch.nn.Linear):
                    # Check if this is likely the final classification layer
                    if module.out_features == 1000:  # Common ImageNet class count
                        ...
                        if parent_name:
                            ...
                            setattr(model, child_name, new_head)
                        
                        print(f"Replaced final classification layer {name}: {in_features} -> {args.num_classes} classes")
                        break
            else:
                print("Warning: Could not find or modify final classification layer. Model may not work correctly.")
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            if args.rank == 0:
                ...
                # Draw and save the loss curve after each epoch
                if output_dir is not None:
